...
const initialize = async (token) => {
  axios.setHeaders({
    ...axios.instance.defaults.headers,
...
const getOrganizationLeaderboard = async ({ orgs = [], filters = {} }) => {
  try {
    if (filters.pagination) {
      let contributions = await utils.getPaginatedResults(
        githubService.getOrganizationPRs,
        orgs,
        filters
      );
      ...
      let contributions = await utils.getNonPaginatedResults(
        githubService.getOrganizationPRs,
        orgs,
        filters
      );
...
const getRepositoryLeaderboard = async ({ owner, repoName, filters = {} }) => {
  try {
    const contributions = await utils.getPaginatedResults(
      githubService.getRepositoryPRs,
      [],
      filters,
      owner,
      repoName
